...
if _HAS_FLASK:
    ...
    def api_analytics_node(node_id):
        ...
        no_mqtt = request.args.get("no_mqtt", "1") != "0"   # default: radio-only
        stats = _node_db.get_per_node_analytics(node_id, hours=hours, hide_broadcast=hide_broadcast, no_mqtt=no_mqtt)
    ...
    @app.route("/api/analytics/avg_hops")
    @login_required
    def api_analytics_avg_hops():
        if not _HAS_NODE_DB:
            return jsonify({})
    ...
    @app.route("/api/analytics/hop_reachability")
    @login_required
    def api_analytics_hop_reachability():
        if not _HAS_NODE_DB:
            return jsonify({})
        hours = float(request.args.get("hours", 4))
        no_mqtt = request.args.get("no_mqtt", "1") != "0"
        return jsonify(_node_db.get_hop_reachability(hours=hours, no_mqtt=no_mqtt))
    ...
    def api_analytics_packets():
        ...
        stats = _node_db.get_packet_analytics(hours=hours, portnum_filter=portnum_filter,
                                               hide_broadcast=hide_broadcast, no_mqtt=no_mqtt)
